\documentclass[12pt,a4paper]{article}

% ── Packages ────────────────────────────────────────────────────────────────
\usepackage[a4paper, margin=2.5cm]{geometry}
\usepackage{graphicx}
\usepackage{hyperref}
\usepackage{hypcap}
\usepackage{bookmark}
\usepackage{amsmath,amssymb,amsfonts}
\usepackage{titlesec}
\usepackage{caption}
\usepackage{subcaption}
\usepackage{enumitem}
\usepackage{lmodern}
\usepackage{cmap}
\usepackage{xcolor}
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows, positioning}
\usepackage{float}
\usepackage{colortbl}


% Soft pastel colors for headings
\definecolor{softblue}{HTML}{6B9AC4}
\definecolor{softteal}{HTML}{7FB3B3}
\definecolor{softpurple}{HTML}{B39EB5}
\definecolor{low}{HTML}{F8E6E6}
\definecolor{mid}{HTML}{E6F2F2}
\definecolor{high}{HTML}{D6EAF8}
\definecolor{best}{HTML}{A9CCE3}
% Table and long table support for large result tables
\usepackage{tabularx}
\usepackage{longtable}
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{array}
\usepackage{fancyhdr}

\usepackage{fontspec}
\usepackage{polyglossia}

\setmainlanguage{english}
\setotherlanguage{arabic}

% Use an Arabic font that also contains Latin glyphs. This prevents square boxes
% if a caption/header/table accidentally inherits the Arabic font after RTL text.
\IfFontExistsTF{DejaVu Sans}{%
  \newfontfamily\arabicfont[Script=Arabic,Scale=MatchLowercase]{DejaVu Sans}%
}{%
  \newfontfamily\arabicfont[Script=Arabic,Scale=MatchLowercase,BoldFont={Noto Sans Arabic},ItalicFont={Noto Sans Arabic}]{Noto Sans Arabic}%
}

\usepackage{comment}	
\usepackage{lmodern}
\usepackage{cmap}
\usepackage{chngcntr}
\usepackage{placeins}
\counterwithin{figure}{section}
\counterwithin{table}{section}


% ── Hyperref setup ───────────────────────────────────────────────────────────
\hypersetup{
	colorlinks=true,
	linkcolor=black,
	urlcolor=black,
	citecolor=black
}

% ── Section formatting (matches PDF style) ───────────────────────────────────
\titleformat{\section}{\large\bfseries\color{softblue}}{\thesection}{1em}{}
\titleformat{\subsection}{\normalsize\bfseries}{\thesubsection}{1em}{}
\titleformat{\subsubsection}{\normalsize\bfseries}{\thesubsubsection}{1em}{}

% ── Header / Footer ──────────────────────────────────────────────────────────
\pagestyle{fancy}
\fancyhf{}
\rhead{\small Neural Networks -- Assignment 3}
\lhead{\small Cairo University}
\cfoot{\thepage}
\setlength{\headheight}{14pt}
\renewcommand{\headrulewidth}{0.4pt}

% ── Graphics path ────────────────────────────────────────────────────────────
\graphicspath{
    {Figures/}
    {../prob_1/Figures/}
	{../prob_2/Figures/}
	{../prob_3/Figures/}
	{../prob_4/Figures/}
	{../prob_5/Figures/}
	{../prob_6/Figures/}
}
\begin{document}

% ── Title Page ───────────────────────────────────────────────────────────────
\begin{titlepage}
\centering
\vspace*{0.5cm}

\makebox[\textwidth][c]{%
\raisebox{-0.4cm}{\includegraphics[width=0.25\textwidth]{Figures/image1.png}}\hspace{2.5cm}%
\includegraphics[width=0.17\textwidth]{Figures/image2.png}%
}

\vspace{0.5cm}
{\Large\bfseries Cairo University\\}
\vspace{0.2cm}
{\large Faculty of Engineering\\}
\vspace{0.2cm}
{\normalsize Electronics and Electrical Communications Engineering\\}

\vspace{1.2cm}
\rule{0.8\textwidth}{1.5pt}\\[0.4cm]
{\LARGE\bfseries Assignment 3}\\[0.3cm]
{\large\bfseries ELC 4028 -- Artificial Neural Networks}\\[0.2cm]
\rule{0.8\textwidth}{1.5pt}

\vspace{1.2cm}

\begin{tabular}{|c|c|l|}
\hline
\textbf{ID} & \textbf{Section} & \textbf{Name} \\
\hline
9220399 & 2 & Shahd Gamal Mahmoud Hussein \\
\hline
9220411 & 2 & Salah El-Din Hassan Hassan Mohamed \\
\hline
9220984 & 4 & Yousef Khaled Omar Mahmoud \\
\hline
\end{tabular}

\vspace{1.2cm}

\textit{Supervised by:}\\[0.3cm]
\textit{Dr.\ Mohsen Rashwan}\\
\textit{Dr.\ Mohamed Motaz}

\vfill
\end{titlepage}

% ── Table of Contents ────────────────────────────────────────────────────────
\setcounter{tocdepth}{3}
\tableofcontents
\newpage
\listoffigures
\newpage

% ---- Report Sections (one per problem) ---------------------------------
\section{Problem 1 VAE Synthetic Data with Low-Data Stabilization}
\input{../prob_1/prob1.tex}
\newpage

\section{Problem 2 GAN Synthetic Data with Low-Data Stabilization}
\input{../prob_2/prob2.tex}
\newpage

\section{Problem 3  Impact of Attention Mechanisms}
\input{../prob_3/prob3.tex}
\newpage

\section{Problem 4 General Information Retrieval}
\input{../prob_4/prob4.tex}
\newpage

% Reset language/font state before Problem 5. This avoids square glyphs in
% captions and headers after Arabic text in the previous sections.
\selectlanguage{english}
\normalfont
\section{Problem 5 Benchmarking Arabic NLP Tasks}
\selectlanguage{english}
\normalfont
\input{../prob_5/prob5.tex}
\selectlanguage{english}
\normalfont
\newpage




\end{document}